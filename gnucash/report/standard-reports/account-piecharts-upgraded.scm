...
(define-module (gnucash report standard-reports account-piecharts-upgraded))
		  ...
		  (sort (filter (lambda (pair) (not (>= 0.0 (car pair))))
            ...
            (if 
             ...
             (begin
               ...
               (gnc:html-chart-set-data! chart (unzip1 combined))
 				...
 				(format
 				 ...
 				 (* 100.0 (/ (car pair) (apply + (unzip1 combined)))))
 			       "")
                       ...
                       combined)))
                 (gnc:html-chart-set-row-labels! chart legend-labels))
...
(define (build-report!
  ...
  (gnc:define-report
    'version 1
    'name name
    'report-guid (string-append "exp-" uuid)
    'menu-path (list gnc:menuname-utility)
    'menu-name (string-append "Experimental " menuname)
    'menu-tip menutip
